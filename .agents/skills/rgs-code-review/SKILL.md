---
name: rgs-code-review
description: "Review kode Luau Roblox untuk kepatuhan arsitektur, DataStore safety, RemoteEvent, dan performa."
---

## Phase 1: 加载目标文件

完整读取目标文件。读取 `VISION.md` 了解项目架构标准。
...
| 文件类型 | 审查重点 |
|----------|----------|
| `.server.luau` | 服务器逻辑、DataStore 安全、RemoteEvent 处理 |
| `.client.luau` | 客户端逻辑、UI 更新、本地效果 |
| `GameConfig.luau` | 数值配置、数据结构、公式定义 |
| `*Manager.luau` | 系统架构、状态管理、生命周期 |
| `*System.luau` | 系统设计、接口定义、模块耦合 |
| `*UI.luau` | UI 架构、响应式更新、性能 |
